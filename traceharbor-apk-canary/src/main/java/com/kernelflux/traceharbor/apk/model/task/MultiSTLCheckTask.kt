...
import com.kernelflux.traceharbor.apk.model.task.TaskFactory.TASK_TYPE_CHECK_MULTISTL
...
class MultiSTLCheckTask(
    ...
    private fun isStlLinked(libFile: File): Boolean {
        val processBuilder = ProcessBuilder(toolnmPath ?: "", "-D", "-C", libFile.absolutePath)
        ...
        while (line != null) {
            ...
            Log.d(TAG, "%s", line)
            if (columns.size >= 3 && columns[1] == "T" && columns[2].startsWith("std::")) {
                return true
            }
        ...
        try {
            ...
            for (libFile in libFiles) {
                if (isStlLinked(libFile)) {
                    Log.i(TAG, "lib: %s has stl link", libFile.name)
                    jsonArray.add(libFile.name)
                }
            }
            val jsonResult = taskResult as TaskJsonResult
            jsonResult.add("stl-lib", jsonArray)
            jsonResult.add("multi-stl", jsonArray.size() > 1)
    ...
    companion object {
        private const val TAG = "TraceHarbor.MultiSTLCheckTask"
    }
}
