...
import io.traceharbor.batterycanary.monitor.AppStats;
...
import io.traceharbor.batterycanary.monitor.feature.MonitorFeature.Snapshot;
...
import io.traceharbor.batterycanary.stats.HealthStatsFeature;
import io.traceharbor.batterycanary.stats.HealthStatsFeature.HealthStatsSnapshot;
import io.traceharbor.batterycanary.stats.HealthStatsHelper;
import io.traceharbor.batterycanary.utils.BatteryCanaryUtil;
import io.traceharbor.batterycanary.utils.Consumer;
import io.traceharbor.batterycanary.utils.Function;
import io.traceharbor.batterycanary.utils.PowerProfile;
import io.traceharbor.batterycanary.utils.RadioStatUtil;
...
public class CompositeMonitors {
    private static final String TAG = "TraceHarbor.battery.CompositeMonitors";
    ...
    @CallSuper
    public void clear() {
        TraceHarborLog.i(TAG, hashCode() + " #clear: " + mScope);
        mBgnSnapshots.clear();
        mDeltas.clear();
        mSamplers.clear();
    ...
    protected CompositeMonitors fork(CompositeMonitors that) {
        TraceHarborLog.i(TAG, hashCode() + " #fork: " + mScope);
        that.clear();
    ...
    public int getCpuLoad() {
        ...
        if (uidJiffies != null) {
            ...
            if (pidJiffies == null) {
                TraceHarborLog.w(TAG, JiffiesSnapshot.class + " should be metrics to get CpuLoad");
                return -1;
            }
    ...
    public int getNorCpuLoad() {
        ...
        if (result.sampleAvg >= sumMax) {
            ...
            for (int[] item : cpuFreqSteps) {
                TraceHarborLog.w(TAG, "NorCpuLoad err: freqs = " + Arrays.toString(item));
            }
        }
    ...
    public void start() {
        TraceHarborLog.i(TAG, hashCode() + " #start: " + mScope);
        mAppStats = null;
    ...
    public void finish() {
        TraceHarborLog.i(TAG, hashCode() + " #finish: " + mScope);
        configureEndDeltas();
        collectStacks();
    ...
    protected void configureSampleResults() {
        for (Map.Entry<Class<? extends Snapshot<?>>, Snapshot.Sampler> item : mSamplers.entrySet()) {
            TraceHarborLog.i(TAG, hashCode() + " " + item.getValue().getTag() + " #pause: " + mScope);
            item.getValue().pause();
            ...
            if (feature != null && mMonitor != null) {
                ...
                sampler = new Snapshot.Sampler("cpufreq", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        TraceHarborLog.i(TAG, CompositeMonitors.this.hashCode() + " #onSampling: " + mScope);
                        TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + ", val = " + list);
                        if (list.isEmpty()) {
            ...
            if (feature != null && mMonitor != null) {
                sampler = new Snapshot.Sampler("batt-temp", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + ", val = " + value);
                        if (value == -1) {
                            return Snapshot.Sampler.INVALID;
                        }
                        return value;
                ...
                sampler = new Snapshot.Sampler("cpu-stat", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        for (int i = 0; i < snapshot.cpuCoreStates.size(); i++) {
                            ...
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " cpuCore" + i  + ", val = " + item.getList());
                        }
                        for (int i = 0; i < snapshot.procCpuCoreStates.size(); i++) {
                            ...
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " procCpuCluster" + i + ", val = " + item.getList());
                        }
                        return 0;
                    }
                });
                ...
                sampler = new Snapshot.Sampler("uid-jiffies", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        if (mLastSnapshot != null) {
                            ...
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " avgUidJiffies, val = " + avgUidJiffies + ", minute = " + minute);
                            for (Delta<JiffiesSnapshot> item : delta.dlt.pidDeltaJiffiesList) {
                                ...
                                TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " avgPidJiffies, val = " + avgPidJiffies + ", minute = " + minute + ", name = " + item.dlt.name);
                            }
                            mLastSnapshot = curr;
            ...
            if (feature != null && mMonitor != null) {
                sampler = new MonitorFeature.Snapshot.Sampler("traffic", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        if (snapshot != null) {
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " wifiRx, val = " + snapshot.wifiRxBytes);
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " wifiTx, val = " + snapshot.wifiTxBytes);
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " mobileRx, val = " + snapshot.mobileRxBytes);
                            TraceHarborLog.i(TAG, "onSampling " + sampler.mCount + " " + sampler.mTag + " mobileTx, val = " + snapshot.mobileTxBytes);
                        }
                        return 0;
                    }
                });
                ...
                sampler = new MonitorFeature.Snapshot.Sampler("health-stats", mMonitor.getHandler(), new Function<Snapshot.Sampler, Number>() {
                    ...
                    public Number apply(Snapshot.Sampler sampler) {
                        ...
                        if (snapshot instanceof HealthStatsSnapshot) {
                            TraceHarborLog.i(TAG, "onAcc " + sampler.mCount + " " + sampler.mTag);
