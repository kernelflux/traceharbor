...
import io.traceharbor.batterycanary.shell.ui.TopThreadIndicator;
import io.traceharbor.batterycanary.utils.BatteryCanaryUtil;
import io.traceharbor.batterycanary.utils.KernelCpuSpeedReader;
import io.traceharbor.batterycanary.utils.KernelCpuUidFreqTimeReader;
import io.traceharbor.batterycanary.utils.PowerProfile;
import io.traceharbor.batterycanary.utils.ProcStatUtil;
...
import static io.traceharbor.batterycanary.utils.BatteryCanaryUtil.JIFFY_MILLIS;
import static io.traceharbor.batterycanary.utils.BatteryCanaryUtil.ONE_HOR;
...
public class CpuStatFeature extends AbsTaskMonitorFeature {
    private static final String TAG = "TraceHarbor.battery.CpuStatFeature";
    ...
    private void tryInitPowerProfile() {
        ...
        synchronized (this) {
            ...
            try {
                ...
                TraceHarborLog.w(TAG, "Init cpuStat failed: " + e.getMessage());
                mPowerProfile = null;
            }
        }
    }
    ...
    public CpuStateSnapshot currentCpuStateSnapshot(int pid) {
        ...
        try {
            ...
            TraceHarborLog.w(TAG, "Read cpu core state fail: " + e.getMessage());
    ...
    public UidCpuStateSnapshot currentUidCpuStateSnapshot() {
        ...
        try {
            ...
            TraceHarborLog.w(TAG, "get curr UidCpuStatSnapshot failed: " + e.getMessage());
            curr.setValid(false);
        }
        return curr;
    }
