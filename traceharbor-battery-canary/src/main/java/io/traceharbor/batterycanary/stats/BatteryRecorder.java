...
public interface BatteryRecorder {
    String TAG = "TraceHarbor.battery.recorder";
    ...
    class MMKVRecorder implements BatteryRecorder {
        ...
        public void write(String date, BatteryRecord record, String proc) {
            try {
                ...
                TraceHarborLog.w(TAG, "record encode failed: " + e.getMessage());
            }
        }
            ...
            for (String item : keys) {
                if (item.startsWith(keyPrefix)) {
                    try {
                        ...
                        TraceHarborLog.w(TAG, "record decode failed: " + e.getMessage());
                    }
                }
            }
            ...
            for (String item : keys) {
                if (item.startsWith(keyPrefix)) {
                    try {
                        ...
                        TraceHarborLog.w(TAG, "record clean failed: " + e.getMessage());
                    }
                }
            }
        }
