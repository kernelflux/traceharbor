...
bool memguard::EndHook(int rule_group_id, const std::vector<std::string>& ignore_pathname_regex_list) {
    ...
    if ((ret = xhook_grouped_ignore(rule_group_id, ".*/libtraceharbor-memguard\\.so$", nullptr)) != 0) {
    ...
    if ((ret = xhook_grouped_ignore(rule_group_id, ".*/libtraceharbor-hookcommon\\.so$", nullptr)) != 0) {
