...
/**
 * Isolated from [TraceHarborTasksManager] so the legacy [com.android.build.gradle.api.BaseVariant] type
 * is not loaded on the classloader when removeUnused is disabled (Gradle 8 + app plugin on classpath
 * can otherwise throw IllegalStateException during configuration).
 */
object TraceHarborRemoveUnusedRegistrar {
    ...
    fun register(android: Any, project: Project, ext: TraceHarborRemoveUnusedResExtension) {
        project.afterEvaluate {
            ...
            if (variants == null) {
                Log.w(TraceHarborTasksManager.TAG, "Android extension does not expose applicationVariants. removeUnusedResources is disabled for now.")
                return@afterEvaluate
            }
            ...
            variants.forEach { rawVariant ->
                ...
                if (Util.isNullOrNil(ext.variant) || variant.name.equals(ext.variant, true)) {
                    Log.i(TraceHarborTasksManager.TAG, "RemoveUnusedResourcesExtension: %s", ext)
                    ...
                    removeUnusedResourcesTaskProvider.configure { task ->
                        ...
                        if (packageDependency != null) {
                            ...
                            Log.w(TraceHarborTasksManager.TAG, "Could not locate package task/provider for variant %s. removeUnusedResources may need AGP-specific follow-up.",
                                variant.name)
                        }
                    }
                }
