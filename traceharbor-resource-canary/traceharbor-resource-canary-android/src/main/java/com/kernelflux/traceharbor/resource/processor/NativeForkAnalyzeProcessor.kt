...
import com.kernelflux.traceharbor.util.safeLetOrNull
import java.io.File
import java.util.*
...
private const val TAG = "TraceHarbor.LeakProcessor.NativeForkAnalyze"
...
private class RetryRepository(private val dir: File) {
    ...
    fun save(hprof: File, activity: String, key: String, failure: String): Boolean {
        try {
            ...
            TraceHarborLog.i(TAG, "Save HPROF analyze retry record ${activity}(${id}).")
...
class NativeForkAnalyzeProcessor(watcher: ActivityRefWatcher) : BaseLeakProcessor(watcher) {
    ...
    companion object {
        private const val RETRY_REPO_NAME = "traceharbor_res_process_retry"
    }
    ...
    private val retryRepo: RetryRepository? by lazy {
        try {
            ...
            TraceHarborLog.printErrStackTrace(TAG, throwable, "Failed to initialize retry repository")
            null
        }
    }
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                watcher.detectHandler.post {
                    ...
                    retryRepo?.process { hprof, keyFile, activity, key, failure ->
                        TraceHarborLog.i(TAG, "Found record ${activity}(${hprof.name}).")
...
//                            }
                        ...
                        if (result.mLeakFound) {
                            ...
                            result.toString().let {
                                ...
                                TraceHarborLog.i(TAG, "retry leak found: $it")
                            }
    ...
    init {
        if (TraceHarborUtil.isInMainProcess(watcher.context)) {
            try {
                IntentFilter().apply {
    ...
    override fun process(destroyedActivityInfo: DestroyedActivityInfo): Boolean {
        ...
        TraceHarborLog.i(TAG, "fork dump and analyse")
        val result = MemoryUtil.dumpAndAnalyze(hprof.absolutePath, key, timeout = 600)
        TraceHarborLog.i(TAG, "fork dump and analyse done")
        if (result.mFailure != null) {
            // Copies file to retry repository and analyzes it again when the screen is locked.
            TraceHarborLog.i(TAG, "Process failed, move into retry repository.")
            ...
            if (result.mLeakFound) {
                ...
                result.toString().let {
                    ...
                    TraceHarborLog.i(TAG, "leak found: $it")
                }
            }
        }
