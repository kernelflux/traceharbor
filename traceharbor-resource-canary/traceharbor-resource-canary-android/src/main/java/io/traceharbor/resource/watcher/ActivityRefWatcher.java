 ...
 */
package io.traceharbor.resource.watcher;
...
import io.traceharbor.lifecycle.EmptyActivityLifecycleCallbacks;
import io.traceharbor.report.FilePublisher;
...
import io.traceharbor.resource.processor.AutoDumpProcessor;
import io.traceharbor.resource.processor.BaseLeakProcessor;
import io.traceharbor.resource.processor.ForkDumpProcessor;
import io.traceharbor.resource.processor.LazyForkAnalyzeProcessor;
import io.traceharbor.resource.processor.ManualDumpProcessor;
import io.traceharbor.resource.processor.NativeForkAnalyzeProcessor;
import io.traceharbor.resource.processor.NoDumpProcessor;
import io.traceharbor.resource.processor.SilenceAnalyseProcessor;
import io.traceharbor.resource.watcher.RetryableTaskExecutor.RetryableTask;
...
public class ActivityRefWatcher extends FilePublisher implements Watcher {
    private static final String TAG = "TraceHarbor.ActivityRefWatcher";
    ...
    private ActivityRefWatcher(Application app,
        ...
        mHandlerThread = TraceHarborHandlerThread.getNewHandlerThread("traceharbor_res", Thread.NORM_PRIORITY); // avoid blocking default matrix thread
    ...
    private final RetryableTask mScanDestroyedActivitiesTask = new RetryableTask() {
        ...
        public Status execute() {
            ...
            if (Debug.isDebuggerConnected() && !mResourcePlugin.getConfig().getDetectDebugger()) {
                TraceHarborLog.w(TAG, "debugger is connected, to avoid fake result, detection was delayed.");
                return Status.RETRY;
            }
...
//                TraceHarborLog.d(TAG, "system ignore our gc request, wait for next detection.");
//                return Status.RETRY;
//            }
            ...
            while (infoIt.hasNext()) {
                ...
                if ((mDumpHprofMode == ResourceConfig.DumpMode.NO_DUMP || mDumpHprofMode == ResourceConfig.DumpMode.AUTO_DUMP)
                    ...
                    TraceHarborLog.v(TAG, "activity with key [%s] was already published.", destroyedActivityInfo.mActivityName);
                    infoIt.remove();
                    continue;
                }
                triggerGc();
                ...
                if (destroyedActivityInfo.mDetectedCount < mMaxRedetectTimes
                    ...
                    TraceHarborLog.i(TAG, "activity with key [%s] should be recycled but actually still exists in %s times, wait for next detection to confirm.",
                ...
                TraceHarborLog.i(TAG, "activity with key [%s] was suspected to be a leaked instance. mode[%s]", destroyedActivityInfo.mKey, mDumpHprofMode);
                ...
                if (mLeakProcessor.process(destroyedActivityInfo)) {
                    TraceHarborLog.i(TAG, "the leaked activity [%s] with key [%s] has been processed. stop polling", destroyedActivityInfo.mActivityName, destroyedActivityInfo.mKey);
                    infoIt.remove();
                }
            }
