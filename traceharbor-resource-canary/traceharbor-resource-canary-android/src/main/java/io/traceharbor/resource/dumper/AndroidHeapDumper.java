 ...
 */
package io.traceharbor.resource.dumper;
...
import io.traceharbor.resource.leakcanary.internal.FutureResult;
import io.traceharbor.resource.R;
...
public class AndroidHeapDumper {
    private static final String TAG = "TraceHarbor.AndroidHeapDumper";
    ...
    public File dumpHeap(boolean isShowToast) {
        ...
        if (hprofDir == null) {
            TraceHarborLog.w(TAG, "hprof file path: %s does not indicate a full path.", hprofFile.getAbsolutePath());
            return null;
        }

        if (!hprofDir.canWrite()) {
            TraceHarborLog.w(TAG, "hprof file path: %s cannot be written.", hprofFile.getAbsolutePath());
            return null;
        }

        if (hprofDir.getFreeSpace() < 1.5 * 1024 * 1024 * 1024) {
            TraceHarborLog.w(TAG, "hprof file path: %s free space not enough", hprofDir.getAbsolutePath());
            return null;
        }
        ...
        if (isShowToast) {
            ...
            try {
                ...
                TraceHarborLog.printErrStackTrace(TAG, e, "failed to dump heap into file: %s.", hprofFile.getAbsolutePath());
                return null;
            }
        } else {
            try {
