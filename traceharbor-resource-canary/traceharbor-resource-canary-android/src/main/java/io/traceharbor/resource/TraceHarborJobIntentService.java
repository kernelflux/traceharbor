...
package io.traceharbor.resource;
 ...
 * library 28.0.0-alpha1. Some changes was made for adapting TraceHarbor's specific usage.
 *
 ...
 *      complete}
 */
public abstract class TraceHarborJobIntentService extends Service {
    static final String TAG = "TraceHarbor.JobIntentService";
    ...
    static final class CompatWorkEnqueuer extends WorkEnqueuer {
        ...
        CompatWorkEnqueuer(Context context, ComponentName cn) {
            ...
            if (mContext.checkPermission(Manifest.permission.WAKE_LOCK, Process.myPid(), Process.myUid()) == PackageManager.PERMISSION_GRANTED) {
                ...
                TraceHarborLog.w(TAG, "it would be better to grant WAKE_LOCK permission to your app so that tinker can use WakeLock to keep system awake.");
    ...
    /**
     * Implementation of a JobServiceEngine for interaction with TraceHarborJobIntentService.
     */
    @RequiresApi(26)
    static final class JobServiceEngineImpl extends JobServiceEngine
            implements TraceHarborJobIntentService.CompatJobEngine {
        static final String TAG = "JobServiceEngineImpl";

        static final boolean DEBUG = false;

        final TraceHarborJobIntentService mService;
        final Object mLock = new Object();
        JobParameters mParams;

        final class WrapperWorkItem implements TraceHarborJobIntentService.GenericWorkItem {
        ...
        JobServiceEngineImpl(TraceHarborJobIntentService service) {
            super(service);
            mService = service;
        }
         ...
         */
        @Override
        public TraceHarborJobIntentService.GenericWorkItem dequeueWork() {
            JobWorkItem work;
            synchronized (mLock) {
                if (mParams == null) {
                    return null;
    ...
    @RequiresApi(26)
    static final class JobWorkEnqueuer extends TraceHarborJobIntentService.WorkEnqueuer {
     ...
     */
    public TraceHarborJobIntentService() {
    ...
    /**
     * Call this to enqueue work for your subclass of {@link TraceHarborJobIntentService}.  This will
