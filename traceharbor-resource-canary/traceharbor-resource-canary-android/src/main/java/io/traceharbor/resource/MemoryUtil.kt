...
private const val TAG = "TraceHarbor.MemoryUtil"

private fun info(message: String) {
    TraceHarborLog.i(TAG, message)
}

private fun error(message: String, throwable: Throwable? = null) {
    if (throwable != null)
        TraceHarborLog.printErrStackTrace(TAG, throwable, message)
    else
        TraceHarborLog.e(TAG, message)
}
...
object MemoryUtil {
    ...
    private val storageDir: File =
        File(TraceHarbor.with().application.cacheDir, "traceharbor_mem_util").apply {
            assureIsDirectory()
        }
    ...
    private val initialized: InitializeException? = run {
        try {
            System.loadLibrary("traceharbor_mem_util")
            loadJniCache()
