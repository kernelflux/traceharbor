...
object HprofFileManager {
    ...
    private const val TAG = "TraceHarbor.HprofFileManager"
    ...
    private val hprofStorageDir by lazy { File(TraceHarbor.with().application.cacheDir.absolutePath + "/hprofs") }
        ...
        safeApply(TAG) {
            ...
            if (hprofStorageDir.exists() && hprofStorageDir.isDirectory) {
                hprofStorageDir.listFiles { it: File ->
                    ...
                    if (it.exists()) {
                        TraceHarborLog.i(TAG, "expired: ${it.absolutePath}")
                        it.delete()
                    }
                }
            }
    ...
    private fun File.lru() {
        ...
        files.forEach {
            TraceHarborLog.d(TAG, "==> list sorted: ${it.absolutePath}, last mod = ${it.lastModified()}")
        }
    ...
    private val processSuffix by lazy {
        if (TraceHarborUtil.isInMainProcess(TraceHarbor.with().application)) {
            "Main"
        } else {
            val split =
                TraceHarborUtil.getProcessName(TraceHarbor.with().application).split(":").toTypedArray()
            if (split.size > 1) {
                split[1]
            } else {
                "unknown"
