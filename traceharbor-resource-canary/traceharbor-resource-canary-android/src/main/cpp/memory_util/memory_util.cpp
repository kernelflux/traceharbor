...
Java_com_kernelflux_traceharbor_resource_MemoryUtil_forkDump(JNIEnv *env, jobject,
    ...
    int task_pid = fork_task("traceharbor_mem_dump", timeout);
    if (task_pid != 0) {
        return task_pid;
    } else {
        /* dump */
...
Java_com_kernelflux_traceharbor_resource_MemoryUtil_forkAnalyze(JNIEnv *env, jobject,
    ...
    int task_pid = fork_task("traceharbor_mem_anlz", timeout);
    if (task_pid != 0) {
        return task_pid;
    } else {
        /* analyze */
...
Java_com_kernelflux_traceharbor_resource_MemoryUtil_forkDumpAndAnalyze(JNIEnv *env, jobject,
    ...
    int task_pid = fork_task("traceharbor_mem_d&a", timeout);
    if (task_pid != 0) {
        return task_pid;
    } else {
        /* dump */
