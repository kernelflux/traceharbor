...
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.HUAWEI
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.LENOVO
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.LG
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.MEIZU
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.MOTOROLA
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.NVIDIA
import com.kernelflux.traceharbor.resource.analyzer.utils.ManufacturerNames.SAMSUNG
import com.kernelflux.traceharbor.resource.analyzer.utils.VersionCodes
 ...
 *
 * Most app developers should use [createAppDefaults]. However, you can also pick the leaks you
 * want to ignore by creating an [EnumSet] that matches your needs and calling [createBuilder]
 */
@Suppress("unused", "MemberVisibilityCanBePrivate") // Public API.
enum class AndroidExcludedRefs {
    // ######## Android SDK Excluded refs ########
    ACTIVITY_CLIENT_RECORD__NEXT_IDLE {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion >= VersionCodes.KITKAT && sdkVersion <= VersionCodes.LOLLIPOP
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.app.ActivityThread\$ActivityClientRecord", "nextIdle")
                .reason(
    ...
    SPAN_CONTROLLER {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion <= VersionCodes.KITKAT
        }
    ...
    MEDIA_SESSION_LEGACY_HELPER__SINSTANCE {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion == VersionCodes.LOLLIPOP
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.media.session.MediaSessionLegacyHelper", "sInstance")
                .reason(
    ...
    TEXT_LINE__SCACHED {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion <= VersionCodes.LOLLIPOP_MR1
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.text.TextLine", "sCached")
                .reason(
    ...
    BLOCKING_QUEUE {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            val reason =
                "Prior to ART, a thread waiting on a blocking queue will leak the last" +
    ...
    INPUT_METHOD_MANAGER__SERVED_VIEW {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion >= VersionCodes.ICE_CREAM_SANDWICH_MR1 && sdkVersion <= VersionCodes.O
        }
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            ...
            excluded.instanceField("android.view.inputmethod.InputMethodManager", "mNextServedView").reason(reason)
            excluded.instanceField("android.view.inputmethod.InputMethodManager", "mServedView").reason(reason)
            excluded
                .instanceField("android.view.inputmethod.InputMethodManager", "mServedInputConnection")
                .reason(reason)
        }
    },
    ...
    INPUT_METHOD_MANAGER__ROOT_VIEW {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion >= VersionCodes.ICE_CREAM_SANDWICH_MR1 && sdkVersion <= VersionCodes.M
        }
    ...
    LAYOUT_TRANSITION {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion >= VersionCodes.ICE_CREAM_SANDWICH && sdkVersion <= VersionCodes.LOLLIPOP_MR1
        }
    ...
    SPELL_CHECKER_SESSION {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return (sdkVersion >= VersionCodes.JELLY_BEAN && sdkVersion <= VersionCodes.LOLLIPOP_MR1) ||
                sdkVersion >= VersionCodes.N
        }
    ...
    ACTIVITY_CHOOSE_MODEL {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            val reason =
                "ActivityChooserModel holds a static reference to the last set" +
            ...
            excluded
                .instanceField("android.support.v7.internal.widget.ActivityChooserModel", "mActivityChoserModelPolicy")
                .reason(reason)
            excluded.instanceField("android.widget.ActivityChooserModel", "mActivityChoserModelPolicy").reason(reason)
        }
    },
    ...
    SPEECH_RECOGNIZER {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.speech.SpeechRecognizer\$InternalListener", "this\$0")
                .reason(
    ...
    ACCOUNT_MANAGER {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.accounts.AccountManager\$AmsTask\$Response", "this\$1")
                .reason(
    ...
    USER_MANAGER__SINSTANCE {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion >= VersionCodes.JELLY_BEAN && sdkVersion <= VersionCodes.M
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.os.UserManager", "mContext")
                .reason(
    ...
    APP_WIDGET_HOST_CALLBACKS {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.appwidget.AppWidgetHost\$Callbacks", "this\$0")
                .reason(
    ...
    CONNECTIVITY_MANAGER__SINSTANCE {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return sdkVersion <= VersionCodes.M
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.net.ConnectivityManager", "sInstance")
                .reason(
    ...
    INSTRUMENTATION_RECOMMEND_ACTIVITY {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return MEIZU == manufacturer && sdkVersion >= VersionCodes.LOLLIPOP && sdkVersion <= VersionCodes.LOLLIPOP_MR1
        }
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                ...
                .reason(
                    "Instrumentation would leak com.android.internal.app.RecommendActivity (in framework.jar)" +
    ...
    DEVICE_POLICY_MANAGER__SETTINGS_OBSERVER {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return MOTOROLA == manufacturer && sdkVersion >= VersionCodes.KITKAT && sdkVersion <= VersionCodes.LOLLIPOP_MR1
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.app.admin.DevicePolicyManager\$SettingsObserver", "this\$0")
                .reason(
    ...
    SPEN_GESTURE_MANAGER {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return SAMSUNG == manufacturer && sdkVersion == VersionCodes.KITKAT
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("com.samsung.android.smartclip.SpenGestureManager", "mContext")
                .reason(
                    "SpenGestureManager has a static mContext field that leaks a reference to the" +
    ...
    GESTURE_BOOST_MANAGER {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return HUAWEI == manufacturer && sdkVersion >= VersionCodes.N && sdkVersion <= VersionCodes.O
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.gestureboost.GestureBoostManager", "mContext")
                .reason(
    ...
    CLIPBOARD_UI_MANAGER__SINSTANCE {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return SAMSUNG == manufacturer && sdkVersion >= VersionCodes.KITKAT && sdkVersion <= VersionCodes.LOLLIPOP
        }
    ...
    SEM_CLIPBOARD_MANAGER__MCONTEXT {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return SAMSUNG == manufacturer && sdkVersion >= VersionCodes.KITKAT && sdkVersion <= VersionCodes.N
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("com.samsung.android.content.clipboard.SemClipboardManager", "mContext")
                .reason(
    ...
    BUBBLE_POPUP_HELPER__SHELPER {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return LG == manufacturer && sdkVersion >= VersionCodes.KITKAT && sdkVersion <= VersionCodes.LOLLIPOP
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.widget.BubblePopupHelper", "sHelper")
                .reason(
                    "A static helper for EditText bubble popups leaks a reference to the latest" +
                        "focused view.",
                )
        }
    },
    ...
    LGCONTEXT__MCONTEXT {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return LG == manufacturer && sdkVersion == VersionCodes.LOLLIPOP
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("com.lge.systemservice.core.LGContext", "mContext")
    ...
    MAPPER_CLIENT {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return NVIDIA == manufacturer && sdkVersion == VersionCodes.KITKAT
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("com.nvidia.ControllerMapper.MapperClient\$ServiceClient", "this\$0")
                .reason(
                    "Not sure exactly what ControllerMapper is about, but there is an anonymous" +
                        " HeapDumpHandler in ControllerMapper.MapperClient.ServiceClient, which leaks" +
    ...
    TEXT_VIEW__MLAST_HOVERED_VIEW {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return SAMSUNG == manufacturer &&
                sdkVersion >= VersionCodes.KITKAT &&
                sdkVersion <= VersionCodes.LOLLIPOP_MR1
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.widget.TextView", "mLastHoveredView")
                .reason(
                    "mLastHoveredView is a static field in TextView that leaks the last hovered" +
                        " view.",
                )
        }
    },
    ...
    PERSONA_MANAGER {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.os.PersonaManager", "mContext")
                .reason(
    ...
    RESOURCES__MCONTEXT {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.content.res.Resources", "mContext")
                .reason(
    ...
    VIEW_CONFIGURATION__MCONTEXT {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .instanceField("android.view.ViewConfiguration", "mContext")
                .reason(
    ...
    SYSTEM_SENSOR_MANAGER_LENOVO {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return LENOVO == manufacturer && sdkVersion == VersionCodes.KITKAT
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.hardware.SystemSensorManager", "mAppContextImpl")
                .reason(
                    "Lenovo specific leak. SystemSensorManager stores a reference to context " +
    ...
    AUDIO_MANAGER__MCONTEXT_STATIC {
        ...
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.media.AudioManager", "mContext_static")
                .reason(
    ...
    ACTIVITY_MANAGER_MCONTEXT {
        override fun accept(sdkVersion: Int, manufacturer: String): Boolean {
            return SAMSUNG == manufacturer && sdkVersion == VersionCodes.LOLLIPOP_MR1
        }

        override fun add(excluded: ExcludedRefs.Builder) {
            excluded
                .staticField("android.app.ActivityManager", "mContext")
                .reason(
    ...
    SOFT_REFERENCES {
        override fun add(excluded: ExcludedRefs.Builder) {
            excluded.clazz(WeakReference::class.java.name).alwaysExclude()
            excluded.clazz(SoftReference::class.java.name).alwaysExclude()
            excluded.clazz(PhantomReference::class.java.name).alwaysExclude()
    ...
    FINALIZER_WATCHDOG_DAEMON {
        override fun add(excluded: ExcludedRefs.Builder) {
            // If the FinalizerWatchdogDaemon thread is on the shortest path, then there was no
            // other reference to the object and it was about to be GCed.
    ...
    MAIN {
        override fun add(excluded: ExcludedRefs.Builder) {
            // The main thread stack is ever changing so local variables aren't likely to hold
            // references for long. If this is on the shortest path, it's probably that there's a
            // longer path with a real leak.
            excluded.thread("main").alwaysExclude()
        }
    },

    // TODO leave works of setting this rule to analyzer caller.
    //    LEAK_CANARY_THREAD {
    //        @Override
    //        void add(ExcludedRefs.Builder excluded) {
    //            excluded.thread("traceharbor_thread").alwaysExclude();
    //        }
    //    },
    EVENT_RECEIVER__MMESSAGE_QUEUE {
        override fun add(excluded: ExcludedRefs.Builder) {
            // DisplayEventReceiver keeps a reference message queue object so that it is not GC'd
            // while the native peer of the receiver is using them.
            // The main thread message queue is held on by the main Looper, but that might be a
            // longer path. Let's not confuse people with a shorter path that is less meaningful.
            excluded
    ...
    open fun accept(sdkVersion: Int, manufacturer: String): Boolean = true

    abstract fun add(excluded: ExcludedRefs.Builder)
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun createAndroidDefaults(sdkVersion: Int, manufacturer: String): ExcludedRefs.Builder {
            return createBuilder(
                EnumSet.of(SOFT_REFERENCES, FINALIZER_WATCHDOG_DAEMON, MAIN, EVENT_RECEIVER__MMESSAGE_QUEUE),
                sdkVersion,
                manufacturer,
            )
        }
         ...
         * This doesn't mean there is no memory leak, to the contrary. However, some leaks are
         * caused by bugs in AOSP or manufacturer forks of AOSP. In such cases, there is very
         * little we can do as app developers except by resorting to serious hacks, so we remove
         * the noise caused by those leaks.
         */
        @JvmStatic
        fun createAppDefaults(sdkVersion: Int, manufacturer: String): ExcludedRefs.Builder {
            return createBuilder(EnumSet.allOf(AndroidExcludedRefs::class.java), sdkVersion, manufacturer)
        }
        ...
        fun createBuilder(
            ...
            for (ref in refs) {
                if (ref.accept(sdkVersion, manufacturer)) {
                    ref.add(excluded)
