...
import com.kernelflux.traceharbor.lifecycle.StatefulOwner
...
class SupervisorService : Service() {
    ...
    companion object {
        private const val TAG = "TraceHarbor.ProcessSupervisor.Service"
        ...
        internal fun start(context: Context) = safeApply(TAG) {
            if (instance != null) {
                TraceHarborLog.e(TAG, "duplicated start")
                return@safeApply
            }
            if (true != ProcessSupervisor.config?.enable) {
                TraceHarborLog.e(TAG, "supervisor was disabled")
                return@safeApply
            }
            val intent = Intent(context, SupervisorService::class.java)
            context.startService(intent)
            TraceHarborLog.i(TAG, "start service")
        }
    ...
    private val binder = object : ISupervisorProxy.Stub() {
        override fun registerSubordinate(
            ...
            runningHandler.post {
                ...
                tokens.forEach {
                    TraceHarborLog.d(TAG, "register: ${it.name}, ${it.statefulName}, ${it.state}")
    ...
    override fun onCreate() {
        ...
        DispatcherStateOwner.observe { stateName, state ->
            if (tokenRecord.isEmpty()) {
                TraceHarborLog.i(TAG, "observe: no other process registered, ignore state changes")
                return@observe
            }
            TraceHarborLog.d(TAG, "supervisor dispatch $stateName $state")
    ...
    override fun onBind(intent: Intent?): IBinder {
        TraceHarborLog.d(TAG, "onBind")
        return binder
    }

    override fun onDestroy() {
        super.onDestroy()
        TraceHarborLog.e(TAG, "SupervisorService destroyed!!!")
        instance = null
    }
    ...
    internal fun backgroundLruKill(
        ...
        val candidate = backgroundProcessLru.firstOrNull {
            it.name != TraceHarborUtil.getProcessName(this)
