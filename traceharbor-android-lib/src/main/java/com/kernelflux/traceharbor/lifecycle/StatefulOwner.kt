...
import com.kernelflux.traceharbor.lifecycle.LifecycleDelegateStatefulOwner.Companion.toStateOwner
...
open class StatefulOwner(val async: Boolean = true) : IStatefulOwner {
     ...
     * You can add observer at any time, even before the initialization of TraceHarbor
     */
    @Synchronized
    override fun observeForever(observer: IStateObserver) {
    ...
    private fun ((observer: IStateObserver) -> Unit).invokeAsync(observer: IStateObserver) {
        ...
        TraceHarborLifecycleThread.executor.execute(object : Runnable {
            override fun run() {
