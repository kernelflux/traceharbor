...
import io.traceharbor.lifecycle.owners.ProcessUIStartedStateOwner
...
private const val TAG = "TraceHarbor.MemoryInfoFactory"
...
object MemInfoFactory {
    ...
    init {
        if (!manualInitialized && !TraceHarbor.isInstalled()) {
            throw IllegalStateException("TraceHarbor is NOT installed or MemoryInfoFactory is not initialized!!!")
        }
    }

    val activityManager = (if (manualInitialized) application else TraceHarbor.with().application)
...
data class ProcessInfo(
    ...
    val name: String = TraceHarborUtil.getProcessName(TraceHarbor.with().application),
...
data class MemInfo(
    ...
    companion object {
        ...
        fun getAllProcessPss(): Array<MemInfo> {
            ...
            TraceHarborLog.i(TAG, "getAllProcessPss cost: ${System.currentTimeMillis() - begin}")
            return memInfoArray
        }
        ...
        private fun prepareAllProcessInfo(): Array<MemInfo> {
            ...
            if (processInfoList == null) {
                TraceHarborLog.e(TAG, "ERROR: activityManager.runningAppProcesses - no running process")
                return emptyArray()
            }

            TraceHarborLog.d(TAG, "processInfoList[$processInfoList]")

            val systemInfo = SystemInfo.get()
            for (i in processInfoList.indices) {
                val processInfo = processInfoList[i]
                val pkgName = TraceHarbor.with().application.packageName
