...
data class SupervisorConfig(
     ...
     * TraceHarbor init in the Supervisor process
     */
...
object ProcessSupervisor : IProcessListener by ProcessSubordinate.processListener {
    ...
    private const val TAG = "TraceHarbor.ProcessSupervisor"
    ...
    val isSupervisor: Boolean by lazy {
        ...
        return@lazy TraceHarborUtil.getProcessName(application!!) == serviceInfo?.processName || SupervisorService.isSupervisor
    }
    ...
    fun init(app: Application, config: SupervisorConfig?): Boolean {
        ...
        if (true != config?.enable) {
            TraceHarborLog.i(TAG, "Supervisor is disabled")
            return false
        }
        application = app
        if (isSupervisor) {
    ...
    private fun initSupervisor(app: Application) {
        ...
        TraceHarborLog.i(tag, "initSupervisor")
    }
    ...
    private fun inCharge(autoCreate: Boolean, app: Application) {
        ...
        val conn = object : ServiceConnection {
            override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
                TraceHarborLifecycleThread.handler.post { // do NOT run ipc in main thread
                    SupervisorPacemaker.uninstall()
                    SubordinatePacemaker.uninstall(app)
                    supervisorProxy = ISupervisorProxy.Stub.asInterface(service)
                    TraceHarborLog.i(tag, "on Supervisor Connected $supervisorProxy")
                    ...
                    ProcessUILifecycleOwner.onSceneChangedListener =
                        object : ProcessUILifecycleOwner.OnSceneChangedListener {
                            override fun onSceneChanged(newScene: String, origin: String) {
                                TraceHarborLog.d(tag, "onSceneChanged: $origin -> $newScene")
            ...
            override fun onServiceDisconnected(name: ComponentName?) {
                TraceHarborLifecycleThread.handler.post {
                    TraceHarborLog.e(tag, "onServiceDisconnected $name")
                    ...
                    safeLet({
                        ...
                        TraceHarborLog.e(tag, "rebound supervisor")
                    }, failed = {
                        // install subordinate pacemaker
                        TraceHarborLog.printErrStackTrace(tag, it, "rebound supervisor failed")
                        SubordinatePacemaker.install(app) {
                            safeApply(tag) {
                                app.bindService(intent, this, BIND_WAIVE_PRIORITY)
                                SubordinatePacemaker.uninstall(app)
                                TraceHarborLog.i(tag, "subordinate pacemaker rebound supervisor")
                            }
                        }
                    })
                }
        ...
        TraceHarborLog.i(tag, "inCharge")
    }

    fun getAllProcessMemInfo() : Array<MemInfo>? {
        if (application == null) {
            TraceHarborLog.e(tag, "Supervisor NOT initialized yet or Supervisor is disabled!!!")
            return null
        }
        if (!isSupervisor) {
            TraceHarborLog.e(tag, "Only support for supervisor process")
            return null
        }
