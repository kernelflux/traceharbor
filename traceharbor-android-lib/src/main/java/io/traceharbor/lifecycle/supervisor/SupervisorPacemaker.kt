...
internal object SupervisorPacemaker : BroadcastReceiver() {
    ...
    private fun installPacemaker(context: Context?) {
        if (pacemaker == null) {
            pacemaker = object : IStateObserver {
                override fun on() {
                    TraceHarborLog.i(ProcessSupervisor.tag, "pacemaker: call supervisor")
            ...
            TraceHarborLog.i(ProcessSupervisor.tag, "pacemaker: install pacemaker")
        }
    }
    ...
    internal fun uninstall() {
        if (pacemaker != null) {
            ...
            pacemaker = null
            TraceHarborLog.i(ProcessSupervisor.tag, "pacemaker: uninstall pacemaker")
        }
    }
    ...
    internal fun install(context: Context?) {
        ...
        if (ProcessSupervisor.isSupervisor) {
            ...
            TraceHarborLog.i(ProcessSupervisor.tag, "pacemaker: receiver installed")
        } else {
    ...
    private fun tellSupervisorForeground(context: Context?) {
        Intent(TELL_SUPERVISOR_FOREGROUND).apply {
            putExtra(KEY_PROCESS_NAME, TraceHarborUtil.getProcessName(context))
        ...
        when (intent?.action) {
            TELL_SUPERVISOR_FOREGROUND -> {
                if (true != ProcessSupervisor.config?.enable) {
                    TraceHarborLog.e(ProcessSupervisor.tag, "supervisor was disabled")
                    return
                }
                if (!ProcessSupervisor.isSupervisor) {
                    TraceHarborLog.e(ProcessSupervisor.tag, "ERROR: this is NOT supervisor process")
                    return
                }
