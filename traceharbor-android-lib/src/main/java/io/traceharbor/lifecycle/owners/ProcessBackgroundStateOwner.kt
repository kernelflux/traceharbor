...
import io.traceharbor.lifecycle.*
 ...
 *      ProcessExplicitBackgroundOwner.addLifecycleCallback(object : ITraceHarborBackgroundCallback() {
 *           override fun onExitBackground() {}
 *           override fun onEnterBackground() {}
 *      })
 *      // auto remove callback when lifecycle destroyed
 *      ProcessExplicitBackgroundOwner.addLifecycleCallback(lifecycleOwner, object : ITraceHarborBackgroundCallback() {
 *           override fun onExitBackground() {}
 *           override fun onEnterBackground() {}
 *      })
 *
 ...
 * then callback [IStateObserver.on] and [ITraceHarborLifecycleCallback.onBackground] would be called
 *
 * NOTICE:
 *
 * [ForegroundServiceLifecycleOwner] is an optional StatefulOwner which is disabled by default and
 * can be enabled by [TraceHarborLifecycleConfig.enableFgServiceMonitor]
 * [OverlayWindowLifecycleOwner] is similar, which can be enabled by [TraceHarborLifecycleConfig.enableOverlayWindowMonitor]
 *
 ...
 * background, the callback [IStateObserver.off] or [ITraceHarborLifecycleCallback.onForeground]
...
object ProcessExplicitBackgroundOwner : StatefulOwner(), IBackgroundStatefulOwner {
    private const val TAG = "TraceHarbor.background.Explicit"
    ...
    var maxCheckInterval = MAX_CHECK_INTERVAL
        set(value) {
            ...
            field = value
            TraceHarborLog.i(TAG, "set max check interval as $value")
        }
    ...
    private val checkTask = object : TimerChecker(TAG, maxCheckInterval) {
        override fun action(): Boolean {
            ...
            if (uiForeground) {
                TraceHarborLog.i(TAG, "turn OFF for UI foreground")
                turnOff() // must be NOT in explicit background and do NOT need polling checker
                return false
            }

            if (!fgService && !visibleWindow) {
                TraceHarborLog.i(TAG, "turn ON")
                turnOn()
                return false
            }
            TraceHarborLog.i(TAG, "turn OFF: fgService=$fgService, visibleView=$visibleWindow, overlay=${OverlayWindowLifecycleOwner.hasOverlayWindow()}")
            turnOff()
            return true
        }
    }
...
object ProcessStagedBackgroundOwner : StatefulOwner(), IBackgroundStatefulOwner {
    private const val TAG = "TraceHarbor.background.Staged"
        ...
        override fun action(): Boolean {
            if (ProcessExplicitBackgroundOwner.active()
                && (ProcessUILifecycleOwner.hasRunningAppTask()
                    .also { TraceHarborLog.i(TAG, "hasRunningAppTask? $it") }
