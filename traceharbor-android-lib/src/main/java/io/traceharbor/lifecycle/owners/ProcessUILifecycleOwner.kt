...
import io.traceharbor.listeners.IAppForeground
import io.traceharbor.util.*
import java.util.*
 ...
 *      ProcessUIStartedStateOwner.addLifecycleCallback(object : ITraceHarborForegroundCallback() {
 *           override fun onEnterForeground() {}
 *           override fun onExitForeground() {}
 *      })
 *      // auto remove callback when lifecycle destroyed
 *      ProcessUIStartedStateOwner.addLifecycleCallback(lifecycleOwner, object : ITraceHarborForegroundCallback() {
 *           override fun onEnterForeground() {}
 *           override fun onExitForeground() {}
 *      })
 *
...
object ProcessUILifecycleOwner {
    ...
    private const val TAG = "TraceHarbor.ProcessLifecycle"
    ...
    internal fun init(app: Application) {
        ...
        packageName = TraceHarborUtil.getPackageName(app)
        ...
        attach(app)
        TraceHarborLog.i(TAG, "init for [${processName}]")
    }

    // ========================== base ========================== //

    private val runningHandler = TraceHarborLifecycleThread.handler
    ...
    private fun isComponentOfProcess(component: ComponentName?, process: String?): Boolean {
        ...
        return process == componentToProcess.getOrPut(component.className) {
            ...
            if (info == null) {
                TraceHarborLog.e(TAG, "got task info not appeared in package manager $info")
                packageName!!
            } else {
                info.processName
            }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                activityManager!!.appTasks
                    .filter {
                        ...
                        TraceHarborLog.i(TAG, "$processName task: ${it.taskInfo.contentToString()}")
                    }.any {
                        TraceHarborLog.d(TAG, "hasRunningAppTask run any")
                        when {
    ...
    var currentFragmentName: String? = null
         ...
         */
        set(value) {
            TraceHarborLog.i(TAG, "[setCurrentFragmentName] fragmentName: $value")
            field = value
    ...
    class DefaultLifecycleObserver : ISerialObserver {
        private fun onDispatchForeground() {
            ...
            TraceHarborLog.i(TAG, "onForeground... visibleScene[$visibleScene@$processName]")
            TraceHarborLifecycleThread.executor.execute {
        ...
        private fun onDispatchBackground() {
            ...
            TraceHarborLog.i(TAG, "onBackground... visibleScene[$visibleScene@$processName]")
            TraceHarborLifecycleThread.executor.execute {
